...
function initHeader() {
  ...
  function syncHeaderState() {
    ...
    header.classList.toggle('menu-open', drawerOpen);
    header.classList.toggle('scrolled', window.scrollY > 30 || drawerOpen);
  }
  ...
  <div class="${wrapClass}" id="lead-form-wrap-${containerId}">
    ...
    <form id="lead-form-${containerId}" class="lead-form-fields">
      ...
      <select required name="service" class="form-select">
        <option value="" disabled ${!defaultService ? 'selected' : ''}>Type of Painting Project</option>
        ${serviceOptions}
...
// Shared controller for the Home and Our Work project galleries.
function initProjectCarousel(root, { autoplayDelay = 5000 } = {}) {
  if (!root) return () => {};
  const track = root.querySelector('.portfolio-grid');
  ...
  let timer;
  let scrollFrame;
  const itemLeft = (index) => items()[index]?.offsetLeft || 0;
  const maxIndex = () => {
    const cards = items();
    const maxScroll = Math.max(0, track.scrollWidth - track.clientWidth);
    let index = 0;
    cards.forEach((card, i) => { if (card.offsetLeft <= maxScroll + 2) index = i; });
    return index;
  };
  const buildDots = () => {
    dots.innerHTML = Array.from({ length: maxIndex() + 1 }, (_, index) =>
      `<button class="gallery-dot" type="button" aria-label="Go to project ${index + 1}"></button>`
    ).join('');
  };
  const nearestIndex = () => {
    const cards = items();
  ...
  const update = (index) => {
    ...
    [...dots.children].forEach((dot, i) => {
      dot.classList.toggle('gallery-dot-active', i === activeIndex);
  ...
  const goTo = (index, behavior = 'smooth') => {
    ...
    track.scrollTo({ left: itemLeft(wrapped), behavior });
    update(wrapped);
  };
  ...
  const start = () => {
    ...
    if (items().length > 1 && !window.matchMedia('(prefers-reduced-motion: reduce)').matches) {
      timer = setInterval(() => goTo(activeIndex + 1), autoplayDelay);
    }
  };
  ...
  dots.addEventListener('click', (event) => {
    ...
    if (!dot) return;
    goTo([...dots.children].indexOf(dot));
    restart();
  });
  prev.addEventListener('click', () => { goTo(activeIndex - 1); restart(); });
  next.addEventListener('click', () => { goTo(activeIndex + 1); restart(); });
  ...
  root.querySelectorAll('.portfolio-img').forEach((img) => {
    img.addEventListener('error', () => {
      img.closest('.portfolio-img-btn')?.classList.add('portfolio-image-error');
      img.remove();
    }, { once: true });
  });
  ...
  const resizeObserver = new ResizeObserver(() => {
    buildDots();
    goTo(Math.min(activeIndex, maxIndex()), 'auto');
  });
  resizeObserver.observe(track);
  update(0);
  start();
  return () => { stop(); resizeObserver.disconnect(); cancelAnimationFrame(scrollFrame); };
}
